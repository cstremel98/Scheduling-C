...
Test(HelperFunctions, response_time_calculation_detailed, .disabled=false) {
  SCHEDULER_STATS* s = get_empty_stats_block();
  PROCESS *p = create_process(2.0f, 0, 1.0f);
  
  mark_start(p, s, 3.0f);
  
  cr_assert(s->num_processes_started == 1);
  cr_assert_float_eq(s->sum_of_response_time, 2.0f, FLOAT_EPSILON);
}

Test(HelperFunctions, turnaround_time_calculation_detailed, .disabled=false) {
  SCHEDULER_STATS* s = get_empty_stats_block();
  PROCESS *p = create_process(2.0f, 0, 1.0f);
  ...
  cr_assert(s->num_processes_completed == 1);
  cr_assert_float_eq(s->sum_of_turnaround_times, 4.0f, FLOAT_EPSILON);
}

Test(HelperFunctions, finalize_stats_with_multiple_processes, .disabled=false) {
  SCHEDULER_STATS* s = get_empty_stats_block();
  
  PROCESS *p1 = create_process(2.0f, 0, 0.0f);
  mark_start(p1, s, 0.0f);
  mark_end(p1, s, 2.0f);
  
  PROCESS *p2 = create_process(2.0f, 0, 1.0f);
  mark_start(p2, s, 2.0f);
  mark_end(p2, s, 4.0f);
  
  finalize_stats(s);
  
  cr_assert_float_eq(s->average_response_time, 0.5f, FLOAT_EPSILON);
  cr_assert_float_eq(s->average_turnaround_time, 2.5f, FLOAT_EPSILON);
}
